...
struct Segment {
    ...
    static std::string toString(const Segment::Enum &seg) {
        switch (seg) {
        case CONST:
            return "constant";
        case ARG:
...
struct Command {
    ...
    static std::string toString(const Command::Enum &cmd) {
        switch (cmd) {
        case ADD:
            return "add";
        case SUB:
...
class VMWriter {
  public:
    ...
    void writeReturn();
    bool write(const std::string &cmd, const std::string &arg1 = "",
