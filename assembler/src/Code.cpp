...
CodeMap destCodes = {{"", "000"},   {"M", "001"},  {"D", "010"},
                     {"MD", "011"}, {"A", "100"},  {"AM", "101"},
                     {"AD", "110"}, {"AMD", "111"}};

CodeMap jumpCodes = {{"", "000"},    {"JGT", "001"}, {"JEQ", "010"},
                     {"JGE", "011"}, {"JLT", "100"}, {"JNE", "101"},
                     {"JLE", "110"}, {"JMP", "111"}};

CodeMap compCodes = {{"0", "0101010"},   {"1", "0111111"},   {"-1", "0111010"},
...
Code::Code(Instruction &instr, SymbolTable &mapping)
    ...
    case A_COMMAND:
        try {
            ...
            value = std::bitset<15>(mappings.getAddress(instr.symbol));
        }
        break;
    case L_COMMAND:
        break;
...
const std::string Code::string() const {
    ...
    case C_COMMAND:
        str << "111" << comp.to_string() << dest.to_string()
            << jump.to_string();
        break;
    case A_COMMAND:
