...
CommandMap addresses = {{"temp", "5"}, {"pointer", "3"}};

CommandMap segments = {
    {"argument", "ARG"}, {"local", "LCL"}, {"this", "THIS"}, {"that", "THAT"}};

CommandMap binaryArithmetic = {
    {"add", "M=D+M"}, {"sub", "M=M-D"}, {"and", "M=M&D"}, {"or", "M=M|D"}};

CommandMap unaryArithmetic = {{"neg", "M=-M"}, {"not", "M=!M"}};

CommandMap comparisons = {{"eq", "EQ"}, {"lt", "LT"}, {"gt", "GT"}};
...
void CodeWriter::writePushPop(const Command &command) {
    ...
    case CommandType::C_PUSH:
        if (cmd == "constant") {
            ...
            writeFromAddress(
                currentFilename + "." + std::to_string(command.arg2), 0);
        } else if (cmd == "temp" || cmd == "pointer") {
            writeFromAddress(addresses.find(cmd)->second, command.arg2);
        } else if (cmd == "argument" || cmd == "local" || cmd == "this" ||
                   cmd == "that") {
    ...
    case CommandType::C_POP:
        ...
        if (cmd == "static") {
            writeToAddress(currentFilename + "." + std::to_string(command.arg2),
                           0);
        } else if (cmd == "temp" || cmd == "pointer") {
...
void CodeWriter::writeCall(const Command &command) {
    auto returnAddr = currentFilename + "." + command.arg1 + ".RET." +
...
void CodeWriter::writeToPointer(const std::string &address,
                                const std::string &val) {
    loadFromPointer(address, "A");
    write("M=" + val);
};

void CodeWriter::loadFromPointer(const std::string &address,
...
void CodeWriter::loadFromAddress(const std::string &address,
                                 const std::string &dest) {
    loadValue(address, "A");
    write(dest + "=M");
};
...
void CodeWriter::write(const std::string &arg) { out << arg << std::endl; };

void CodeWriter::writeBootstrap() {
    Command initCommand{};
    initCommand.type = C_CALL;
    initCommand.arg1 = "Sys.init";
    initCommand.arg2 = 0;
...
void CodeWriter::equalityFn(const std::string &label,
